 ...
 *
 *Then each range is divided into 2 so we can aachieve 16 frecuency groups
 *
 */
...
public class AudioController : MonoBehaviour
{
    ...
    float[] freqBand   = new float[16]; //Array that stores the audio bands without smoothing

    public float[] bandBuffer = new float[16];//Array that stores the smoothed audio bands

    public float[] audioBandBuffer = new float[16]; //Array that stores the normalized values

    float[] freqBandMax = new float[16]; //Array that stores the current maximum value obtained. 
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 16; i++) //Fills of zeros the max array
            freqBandMax[i] = 0;
    }
    ...
    void MakeFrequencyBand()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int k = 0; k < 2; k++)//Makes it twice in order to have 16 bands
            {
