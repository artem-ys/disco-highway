...
Shader "Toony Colors Pro 2/Examples/Material Layers/Snow + Sand Advanced"
{
	...
	SubShader
	{
		...
		TCP2_TEX2D_WITH_SAMPLER(_MainTex_sand);
		...
		void surf(Input input, inout SurfaceOutputCustom output)
		{
			...
			float4 __normalMap = ( TCP2_TEX2D_SAMPLE(_BumpMap, _BumpMap, input.texcoord0.xy * _MainTex_ST.xy + _MainTex_ST.zw).rgba );
			float4 __normalMap_snow = ( TCP2_TEX2D_SAMPLE(_BumpMap_snow, _BumpMap_snow, input.texcoord0.xy * _BumpMap_snow_ST.xy + _BumpMap_snow_ST.zw).rgba );
			float4 __normalMap_sand = ( float4(0.5019608,0.5019608,1,1) );
			float __bumpScale = ( _BumpScale );
			float __bumpScale_snow = ( _BumpScale_snow );
			float4 __albedo = ( TCP2_TEX2D_SAMPLE(_MainTex, _MainTex, input.texcoord0.xy * _MainTex_ST.xy + _MainTex_ST.zw).rgba );
			float4 __albedo_snow = ( _Albedo_snow.rgba );
			float4 __albedo_sand = ( TCP2_TEX2D_SAMPLE(_MainTex_sand, _MainTex_sand, input.texcoord0.xy * _MainTex_sand_ST.xy + _MainTex_sand_ST.zw).rgba );
			...
			float __noise_snow = (  saturate( TCP2_TEX2D_SAMPLE(_NoiseTexture_snow, _NoiseTexture_snow, input.texcoord0.xy * _NoiseTexture_snow_ST.xy + _NoiseTexture_snow_ST.zw).r * imp_100 ) - imp_100 / 2.0 );
			...
			float __noise_sand = (  saturate( (1 - TCP2_TEX2D_SAMPLE(_NoiseTexture_sand, _NoiseTexture_sand, input.texcoord0.xy * _MainTex_ST.xy + _MainTex_ST.zw).a) * imp_101 ) - imp_101 / 2.0 );
